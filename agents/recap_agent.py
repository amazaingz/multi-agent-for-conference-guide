...
def get_recap_info(query: str, user_id: str = None) -> str:
    """
    Process queries related to post-conference summaries and roadshows.
    Integrates Knowledge Base for summaries and real-time tools for roadshow logistics (weather/dining).

    Args:
        query: User's query about conference recap or roadshows
        user_id: Current user_id (optional)

    Returns:
        Comprehensive answer.
    """
    formatted_query = f"""请综合处理关于 re:Invent 会后回顾和路演的问题：{query}

注意：
1. **知识库定位**：主要用于回答大会总结、技术发布回顾等历史信息。
2. **路演安排**：如果用户咨询路演（Roadshow）的具体安排，且知识库中没有相关信息，请明确告知这一点，但不要停留在那里。
3. **主动服务**：即使用户没有明确问，也请主动询问或建议用户关注路演城市的天气（使用 get_realtime_weather）和周边美食（使用 search_nearby_restaurants）。
4. **场景化建议**：如果用户提到了具体的路演城市（如北京、上海等），请直接调用天气和餐饮工具，为用户提供该城市的实用参会建议。
5. 综合回答：将知识库的回顾信息与实时的生活服务信息（天气、餐饮）结合起来，提供有温度的建议。
"""
